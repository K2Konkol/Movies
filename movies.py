...
from classes.HelperClasses import Parser, Highscore, CompareAwards, CompareNumeric
...
class Main():
    def __init__(self):
        ...
        self.repo = Repository(db=self.db, parser=self.award_parser, highscore=self.highscore)
        ...
        self.parser.add_argument("-f", "--filter_by", help="Filtering by column. Options: awarded, nominated, earned, director [name], actor [name], language [lang]", action='store', nargs='*', type=str)
        self.parser.add_argument("--highscores", help="Show Highscores", action='store_true')
        self.parser.add_argument("-c", "--compare", help="Compare two movie titles by given column. Options: imdb_rating, box_office, awards, runtime.", action='store', nargs='*', type=str)
