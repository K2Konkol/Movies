...
class Highscore():
    ...
    def get_highest_runtime(self, movies):
        ...
        for movie in movies:
            if movie[1] is not None and int(self.parser.str_to_int(movie[1])) > int(self.parser.str_to_int(highest_runtime[1])):
    ...
    def get_highest_box_office(self, movies):
        ...
        for movie in movies:
            if movie[2] is not None and int(self.parser.str_to_int(movie[2])) > int(self.parser.str_to_int(highest_box_office[1])):
    ...
    def get_oscars_highscore(self, movies):
        ...
        for movie in movies:
            oscars = self.parser.get_oscars(movie[3]) if movie[3] is not None else 0
    ...
    def get_nominations_highscore(self, movies):
        ...
        for movie in movies:
            nominations = self.parser.get_nominations(movie[3]) if movie[3] is not None else 0
    ...
    def get_awards_highscore(self, movies):
        ...
        for movie in movies:
            awards = self.parser.get_awards(movie[3]) if movie[3] is not None else 0
...
class CompareNumeric(Compare):
    def compare(self):
        return self.movie1[0] if self.parser.str_to_int(self.movie1[1]) > self.parser.str_to_int(self.movie2[1]) else self.movie2[0]

class CompareAwards(Compare):
    def compare(self):
        return self.movie1[0] if self.parser.get_awards(self.movie1[1]) > self.parser.get_awards(self.movie2[1]) else self.movie2[0]
